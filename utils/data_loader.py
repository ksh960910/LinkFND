...
def read_cf_amazon(file_name):
    f = dict()
    inter_mat = np.loadtxt(file_name, dtype=np.int32) # [u_id, i_id]
    for ins in inter_mat:
        if ins[1] not in f:
            f[ins[1]] = []
        f[ins[1]].append(ins[0])
    ...
    return inter_mat, popular, normal, unpopular
...
def statistics(train_data, valid_data, test_data):
    ...
    if dataset != 'yelp2018' and dataset!='gowalla' and dataset!='amazon-book' and dataset!='toy':
        n_items -= n_users
...
def load_data(model_args):
    ...
    train_cf, popular, normal, unpopular = read_cf(directory + 'train.txt')
    test_cf, _, _, _ = read_cf(directory + 'test.txt')
    if args.dataset != 'yelp2018' and args.dataset!='gowalla' and args.dataset!='amazon-book' and args.dataset!='toy':
        valid_cf, _, _, _ = read_cf(directory + 'valid.txt')
    else:
        valid_cf = test_cf
    ...
    user_dict = {
        ...
        'valid_user_set': valid_user_set if args.dataset != 'yelp2018' and args.dataset!='gowalla' and args.dataset!='amazon-book' and args.dataset!='toy' else None,
    ...
    return train_cf, user_dict, n_params, norm_mat, popular, normal, unpopular
